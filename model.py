...
def init_net(feature_size, label_size):
    ...
    with tf.name_scope('network'):
        ...
        # hidden_stack=position_encoding(hidden_stack,tree_embedding_size*num_conv)
        code_vector = bi_directional_rnn_layer(hidden_stack, subtree_len_list)  # 双向循环神经网络
        # code_vector=rnn_layer(hidden_stack,subtree_len_list) #单向循环神经网络
        # code_vector=multi_rnn_layer(hidden_stack,subtree_len_list,rnn_layers)  #多层循环神经网络
...
# 单向循环神经网络
# def rnn_layer(batch_sub_tree_embeddings, subtree_len):
...
#         cell = tf.contrib.rnn.MultiRNNCell([single_rnn_cell() for _ in range(rnn_layers)])
#         return cell
#
#     with tf.variable_scope('encoder'):
#         # batch_statement_embeddings = tf.expand_dims(batch_statement_embeddings, 0)
#         encoder_cell = _create_rnn_cell()
#         encoder_outputs, encoder_state = tf.nn.dynamic_rnn(encoder_cell, batch_sub_tree_embeddings,
#                                                            sequence_length=subtree_len,
#                                                            dtype=tf.float32)
#
#         code_vector = tf.nn.avg_pool(encoder_outputs, axis=1)
#     return code_vector
...
#
#     # 向量位置
#     position_size=tf.to_double(position_size, name='ToDouble')
#     vector_index = 1. / tf.pow(tf.to_double(10000.,name='ToDouble'),  2. * tf.range(1.0, position_size / 2+1, dtype=tf.float64) / position_size)
#     vector_index = tf.expand_dims(vector_index, 0)
#
#     # ast奇偶位置
#     ast_odd_index = tf.range(0.0, tf.cast(seq_len, tf.float64), delta=2, dtype=tf.float64)
#     ast_odd_index = tf.expand_dims(ast_odd_index, 1)
#
#     ast_even_index = tf.range(1.0, tf.cast(seq_len, tf.float64), delta=2, dtype=tf.float64)
#     ast_even_index = tf.expand_dims(ast_even_index, 1)
#
#     # 进行矩阵乘法分别得到两个位置的结果
#     odd_result=tf.matmul(ast_odd_index,vector_index)
#     even_result=tf.matmul(ast_even_index,vector_index)
#
#     # 将两个位置的结果进行连接，具体的方法是，每个矩阵先拓展最后一维，然后进行拼接，最后取这个三维张量的第一维
#     # odd_result=tf.expand_dims(odd_result,-1)
#     # even_result=tf.expand_dims(even_result,-1)
#     position_embedding=tf.concat([odd_result,even_result],-1)
#
#     # 得到这个批次的
#     # position_embedding = tf.expand_dims(position_embedding, 0) \
#     #                      + tf.zeros((batch_size, seq_len, position_size),dtype=tf.float64)
#
#
#     position_embedding = tf.expand_dims(position_embedding, 0) \
#                          + tf.zeros((batch_size, seq_len, tf.to_int32(position_size)),dtype=tf.float32)
#
#     inputs=tf.to_float32(inputs)
#
#     return tf.add(inputs,position_embedding)
...
# 多层循环神经网络堆叠
# 多层动态循环神经网络
def multi_rnn_layer(batch_subtree_embeddings, subtree_len, layers_num):
    stacked_rnn = []
