...
def init_net(feature_size, label_size):
    ...
    with tf.name_scope('network'):
        conv1 = conv_layer(num_conv, tree_embedding_size, nodes[0], children[0], feature_size)
        code_vector = pooling_layer(conv1)

        prediction = softmax_layer(code_vector, code_vector_size, label_size)

    with tf.name_scope('summaries'):
        tf.summary.scalar('tree_size', tf.shape(nodes)[2])
        tf.summary.scalar('child_size', tf.shape(children)[3])

    return nodes, children, code_vector, prediction
...
def pooling_layer(nodes):
    ...
    with tf.name_scope('map'):
        w1 = tf.Variable(tf.truncated_normal([tree_embedding_size*num_conv, code_vector_size], stddev=0.1))
        b1 = tf.Variable(tf.zeros([code_vector_size]))
        code_vector = tf.nn.tanh(tf.matmul(pooled, w1) + b1)
